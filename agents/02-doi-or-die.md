...
description: "Murderboard role 2 of 11, Citation & reference validator — 'DOI or Die.' For every reference or named attribution, confirm the work exists, says what is quoted, and is the origin — traced back until the citations stop, and forward to what the same authors did next (web search / DOI where needed)."
...
**The process file grants you `Read, Grep, Glob, Bash, WebSearch, WebFetch`, and nothing else.**
Compare that against the tools you were actually given, then open your output with exactly one of
these two lines:
