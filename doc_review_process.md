...
a rule that depends on being remembered is not a gate. It is written down anyway, because the
alternative is worse than a missing gate. A reviewer who declines a late run currently has no
sanctioned way to say so, and *"I don't want to know at this stage"* reads as nerve failure
rather than as the correct pricing of a real cost. **Naming a legitimate reason to decline after
submission is what makes the answer before submission cheap.** Without one, people do not decline
the timing; they avoid the subject, and the tool goes unused on exactly the documents that would
repay it most.
