...
| [`murderboard_prose.sh`](murderboard_prose.sh) | The prose gate. Runs the **mechanical half of role 5** over a document — every banned construction with its line number, and a word/sentence count per block — **0 clean · 1 hits · 2 unknown**. The word list is **derived** from role 5 at run time, so editing the process file moves the gate, and a reworded list stops it rather than leaving it searching for the old one. It exists because a greppable check sitting inside a judgment role gets answered in prose: *"the wording is clean"* from a reviewer who searched and from one who did not are the same sentence. It deliberately **cannot judge** — it reports that a block is 220 words, never that the block is too long, and which sentence carries the payload stays with role 5. `--selftest` proves every check can still fire, including the ones that must come back **empty**: a gate that cannot report clean is as useless as one that cannot report a hit. |
