...
2. **Wire the gates so they fire without being remembered.** Freshness at session start
   ...
   bash tools/murderboard_subagents.sh --hook --explain  # SessionStart: silent unless a
                                                    #   subagent blocker is visible on disk
   bash tools/murderboard_prose.sh DRAFT.md         # role 5's search: 1 if it found something
   bash tools/murderboard_roster.sh check --require-execution REPORT.md
                                                    # after a run: 1 if a role is missing, or
                                                    #   if the report will not say whether it
                                                    #   ran as subagents or as a single pass
   ```
   The subagent line is optional and the only one here that is an early warning rather than a
